  ...
  <div className="max-w-5xl mx-auto p-6 bg-brand-contentBg text-brand-secondaryText">
    <div className="grid md:grid-cols-2 gap-8">
      ...
      <div className="space-y-4">
        <h1 className="text-3xl font-bold text-brand-title">{product.name}</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          {product.mrp && (
            <span className="line-through text-brand-liteGray">₹{product.mrp}</span>
          )}
    ...
    <div className="mt-10">
      ...
      {loadingRatings ? (
        <div className="space-y-2">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="h-20 bg-brand-secondary rounded animate-pulse" />
          ))}
        </div>
      ) : ratings.length > 0 ? (
        <div className="space-y-4">
            ...
            {[1, 2, 3, 4, 5].map((num) => (
              <StarIcon
                ...
                className={`w-7 h-7 cursor-pointer transition ${
                  num <= userRating ? "text-yellow-400" : "text-brand-liteGray"
                }`}
